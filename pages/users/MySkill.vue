<template>
  <div>
    ...
    <button
      class="button"
      type="is-dark"
      @click="IsUserSkills ? UpdateMySkills() : CreateMySkills()"
    >
      マイスキルを追加する
    </button>
  </div>
...
export default {
  ...
  methods: {
    ...
    CheckExistTags() {
      ...
      const result = OldTags.some(
        (OldTag) => OldTag.tag_name === NewTags[LastIndex].tag_name
      );
      if (result) {
        console.log(result);
    ...
    ChangeSkillTagsArrays(index, skill_id, tag, e) {
      ...
      if (skill_tags.length === 0) {
        ...
        const result = skill_tags.some(
          (skill_tag) => skill_tag[0] === skill_id
        );
        if (result) {
    ...
    CreateMySkills() {
      ...
      this.$axios
        .$post("/api/v1/myskill/user_skill_categories", {
          user_skill_category: {
    ...
    UpdateMySkills() {
      ...
      this.$axios
        .$patch("/api/v1/myskill/user_skill_categories", {
          user_skill_category: {
            skill_arrays: this.skill_tags,
