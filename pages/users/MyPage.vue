     ...
     <h2><strong>基本情報</strong></h2>
      <article class="media">
        ...
        <div class="media-content">
          <div class="content">
            <p>
              <strong>ユーザー名:{{user.username}}</strong><br>  
              <small>性別:{{user.gender}}</small>
              <br>
              <!-- <img :src="user" alt=""> -->
              {{user.description}}
            </p>
            <img v-if="user.wantedly_id" src="~/assets/image/wantedly_mark.png" style="height: 40px;" alt="">
            <img v-if="user.twitter_id" src="https://img.icons8.com/color/48/000000/twitter-circled.png" style="height: 40px;">
            <img v-if="user.github_id" src="https://img.icons8.com/fluent/48/000000/github.png" style="height: 40px;"/>
            <img v-if="user.facebook_id" src="https://img.icons8.com/fluent/48/000000/facebook-new.png" style="height: 40px;"/>
          </div>
...
<div v-for="(site, index) in this.sites" :key="index" style="height: 260px; width:360px; box-shadow: 0 1px 16px 0 rgba(0, 0, 0, 0.15);　display: inline-block; margin: 20px 20px; text-align: center; position: relative;">
        <nuxt-link :to="`/sites/${site.id}`">
          ...
          <img class="is-rounded" :src="site_images[index]" alt="Image" style="">
        </nuxt-link>  
        <div>{{site.id}}
...
export default {
  middleware: 'auth',
    ...
    methods: {
        ...
        FetchFavoriteSite () {
        console.log("aaaaaa")
        this.$axios.$get('/api/v1/user_favorites')
        .then(res => {   
